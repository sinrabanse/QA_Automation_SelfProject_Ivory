export async function mockPaymentHandler(page, testLocators) {
  // Перехватываем все запросы на странице и внутри фреймов
  ...
  if (!frameElementHandle) {
    throw new Error("Не удалось найти фрейм по указанному XPath");
  }

  // Получаем фрейм через элемент handle
  const frame = await frameElementHandle.contentFrame();

  if (!frame) {
    throw new Error("Не удалось получить фрейм из элемента");
  }

  // Выполняем действия в фрейме
