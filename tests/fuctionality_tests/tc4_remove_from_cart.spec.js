...
const stepActions = {
  ...
  "Delete product from cart": async (page) => {
    await page.locator(testLocators.removeFromCartButton).click();
  },
  "Verify results": async (page) => {
    await expect(page.locator(testLocators.emptyCartLocator)).toHaveText(
      testAssertions.empty_cart_text
    );
  },
};
...
const tc = testCases.functionality_tests.find((tc) => tc.id === "4");
